...
<jsp:include page="/header.jsp">
	...
	<div class="products">	 
		<div class="container">
			<h2><c:choose><c:when test="${empty type}">全部系列</c:when><c:otherwise>${type.name}</c:otherwise></c:choose></h2>
...
<a class='btn btn-info' <c:if test="${page.pageNumber==1}">disabled</c:if>
   <c:if test="${page.pageNumber!=1}">href="${pageContext.request.contextPath}/goods_list?pageNo=1&id=${id}"</c:if> >首页</a>
<a class='btn btn-info' <c:if test="${page.pageNumber==1}">disabled</c:if>
   <c:if test="${page.pageNumber!=1}">href="${pageContext.request.contextPath}/goods_list?pageNo=${page.pageNumber-1}&id=${id}"</c:if>>上一页</a>
...
<a class='btn btn-info' <c:if test="${page.totalPage==0||page.pageNumber==page.totalPage}">disabled</c:if>
   <c:if test="${page.pageNumber!=page.totalPage}">href="${pageContext.request.contextPath}/goods_list?pageNo=${page.pageNumber+1}&id=${id}"</c:if>>下一页</a>
<a class='btn btn-info' <c:if test="${page.totalPage==0||page.pageNumber==page.totalPage}">disabled</c:if>
   <c:if test="${page.pageNumber!=page.totalPage}">href="${pageContext.request.contextPath}/goods_list?pageNo=${page.totalPage}&id=${id}"</c:if>>尾页</a>
