...
<div class="container-fluid">
	...
	<ul role="tablist" class="nav nav-tabs">
        <li <c:if test="${status==0}">class="active" </c:if> role="presentation"><a href="${pageContext.request.contextPath}/admin/order_list?status=0">全部订单</a></li>
	...
	<c:forEach items="${page.list}" var="order">
		<tr>
			...
			<td>
				...
				<c:if test="${order.status==3}">
					<a class="btn btn-success" href="${pageContext.request.contextPath}/admin/order_status?id=${order.id}&status=4">完成</a>
				</c:if>
				<a class="btn btn-danger" href="${pageContext.request.contextPath}/admin/order_delete?id=${order.id}&pageNo=${page.pageNumber}&status=${status}">删除</a>
			</td>
       	</tr>
	</c:forEach>
</table>
